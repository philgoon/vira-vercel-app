-- Fix: Prioritize vendors with project experience and ratings
-- Issue: Current function returns vendors without any rated projects
-- Solution: Filter for vendors with performance data and sort by experience
...
BEGIN
  ...
  WHERE v.status = 'active'
    ...
    -- CRITICAL FIX: Only show vendors with project experience
    AND vp.rated_projects > 0
    AND vp.avg_overall_rating IS NOT NULL
...
COMMENT ON FUNCTION match_vendors_hybrid IS 'Hybrid vendor matching: 40% semantic + 40% performance + 20% availability. Only returns vendors with project experience.';
